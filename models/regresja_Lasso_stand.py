...
X_walid_poly = poly.transform(X_walid_scaled)
X_test_poly = poly.transform(X_test_scaled)

model = Lasso(alpha=0.1, max_iter=10000)
model.fit(X_train_poly, y_train)

y_walid_pred = model.predict(X_walid_poly)
...
coefficients = pd.DataFrame({
    "Variable": poly.get_feature_names_out(input_features=zmienne_niezależne),
