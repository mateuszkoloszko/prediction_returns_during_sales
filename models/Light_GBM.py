...
lgbm_model.fit(X_train_data, y_train)
preds_lgbm = lgbm_model.predict(X_walid_data)
...
wyniki = pd.DataFrame(
    data = {
        'RSS': [
            ...
            np.sum(np.square(preds_lgbm - y_walid))
        ],
        'R2': [
            r2_score(y_walid, preds_poly),
            r2_score(y_walid, preds_lgbm)
        ],
        'MAE': [
            mean_absolute_error(y_walid, preds_poly),
            mean_absolute_error(y_walid, preds_lgbm)
        ]
    },
    index=['PolyReg', 'LGBM']
)
...
lgbm_model2 = lightgbm.LGBMRegressor(learning_rate=0.02)
lgbm_model2.fit(X_train_data,y_train)
preds_lgbm2 = lgbm_model2.predict(X_walid_data)
...
wyniki2 = pd.DataFrame(
    data = {
        ...
        'R2': [
            r2_score(y_walid, preds_poly),
            r2_score(y_walid, preds_lgbm2)
        ],
        'MAE': [
            mean_absolute_error(y_walid, preds_poly),
            mean_absolute_error(y_walid, preds_lgbm2)
        ]
    },
    index=['PolyReg', 'LGBM']
)
