...
def remove_outliers_iqr(df_retail):
    results = {}
    cleaned_df = df_retail.copy()
    outlier_indices = set()
    ...
    for column in cleaned_df.select_dtypes(include=["number"]).columns:
        ...
        column_outliers = cleaned_df[(cleaned_df[column] < lower_bound) | (cleaned_df[column] > upper_bound)].index
        outlier_indices.update(column_outliers)
        percent_outliers = len(column_outliers) / len(df_retu) * 100

        results[column] = {
            "Liczba danych odstających": len(column_outliers),
            "Procent danych odstających": percent_outliers,
            "Granice": (lower_bound, upper_bound),
            "Dane odstające": cleaned_df.loc[column_outliers, column].values.tolist()
        }

    outlier_indices = list(outlier_indices)
    total_outliers = df_retu.loc[outlier_indices]

    print(f"Shape dla danych odstających: {total_outliers.shape}")
    print(f"Shape dla wyczyszczonych danych: {cleaned_df.shape}")
    
    return cleaned_df, results

df_cleaned, outlier_results = remove_outliers_iqr(df_retail)
